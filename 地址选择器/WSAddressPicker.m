...
@interface WSAddressPicker()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation WSAddressPicker
...
- (void)setBaseView {
    ...
    UIView *selectView = [[UIView alloc] initWithFrame:CGRectMake(0, height - 330, width, 30)];
    ...
    self.pickView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, height - 300 , width,  300)];
...
- (void)dateCancleAction {
    if (self.delegate && [self.delegate respondsToSelector:@selector(WSAddressPickerCancleAction)]) {
        [self.delegate WSAddressPickerCancleAction];
    }
}

- (void)dateEnsureAction {
    if (self.delegate && [self.delegate respondsToSelector:@selector(WSAddressPickerWithProvince:city:area:)]) {
        [self.delegate WSAddressPickerWithProvince:self.province city:self.city area:self.area];
    }
}
